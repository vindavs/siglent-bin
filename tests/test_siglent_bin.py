...
def test_offset_binary_not_signed():
    ...
    with open(path, 'rb') as f:
        as_signed = np.frombuffer(f.read(), dtype='<i2',
                                  offset=siglent_bin.HEADER_BYTES).astype(np.int64)
...
def test_find_group():
    ...
    assert [os.path.basename(p) for p in paths] == [
        'sds814x_C1_30.bin', 'sds814x_C2_30.bin',
        'sds814x_C3_30.bin', 'sds814x_C4_30.bin']
...
def test_malformed_files_rejected():
    ...
    cases = {
        ...
        'magnitude index out of range':
            good[:0x20] + struct.pack('<i', 999) + good[0x24:],
